    ...
    <div class="container-fluid">
        <div class="main-header-left ">
            <div class="responsive-logo">
                <a href="{{ url('/') }}"><img src="{{ URL::asset('app/storage/assets/img/legal/logo/' . $setting->logo_img_name) }}" class="logo-1" alt="logo"></a>
				<a href="{{ url('/') }}"><img src="{{ URL::asset('app/storage/assets/img/legal/logo/' . $setting->logo_img_name) }}" class="dark-logo-1" alt="logo"></a>
				<a href="{{ url('/') }}"><img src="{{ URL::asset('app/storage/assets/img/legal/logo/' . $setting->logo_img_name) }}" class="logo-2" alt="logo"></a>
				<a href="{{ url('/') }}"><img src="{{ URL::asset('app/storage/assets/img/legal/logo/' . $setting->logo_img_name) }}" class="dark-logo-2" alt="logo"></a>
            </div>
				...
				<div class="dropdown nav-item main-header-notification">
					...
					<div class="dropdown-menu">
						<div class="menu-header-content bg-primary text-right">
							...
							<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">لديك {{ auth()->user()->unreadNotifications->count() }}  تنبيه غير مقروء</p>
							@if(auth()->user()->unreadNotifications->count() > 0)
								<span class="badge badge-pill badge-warning mr-auto my-auto float-right">اضغط على التنبيه لازالته من القائمة </span>
							@endif
						</div>
						<div class="main-notification-list Notification-scroll">
							@foreach(auth()->user()->unreadNotifications as $notification)
								@php 
								$adminName = App\Models\Admin::find($notification->data['admin_id']);
								@endphp
								<a class="d-flex p-3 border-bottom" href="{{ route('admin.markAsReadAndRedirect', $notification->id)}}">
									<div class="mr-3">
										<h5 class="notification-label mb-1">{{ $notification->data['description'] }}</h5>
										<div class="notification-subtext">عن طريق - {{$adminName->name}}</div>
						...
						@if(auth()->user()->unreadNotifications->count() > 0)
							<div class="dropdown-footer">
								<a href="{{ route('admin.showAllNotifications')}}">مشاهدة الكل</a>
							</div>
						@endif
					</div>
				</div>
